...
const nav = [['technology.html','Technology'],['company.html','Company'],['contact.html','Connect']];
...
for (const page of pages) {
 ...
 const header = `<a class="skip-link" href="#main">Skip to content</a><header class="site-header" data-header><a class="brand" href="index.html" aria-label="Argys home"><img src="assets/argys-logo.png" width="756" height="416" alt="Argys — Vision in Surgery"></a><nav class="desktop-nav" aria-label="Main navigation">${nav.map(([url,label])=>`<a href="${url}" ${page===url?'aria-current="page"':''} class="${url==='contact.html'?'nav-cta':''}">${label}${url==='contact.html'?'<span aria-hidden="true">↗</span>':''}</a>`).join('')}</nav><button class="menu-toggle" aria-controls="mobile-nav" aria-expanded="false"><span>Menu</span><i aria-hidden="true"></i></button><nav class="mobile-nav" id="mobile-nav" aria-label="Mobile navigation" hidden>${nav.map(([url,label])=>`<a href="${url}" ${page===url?'aria-current="page"':''}>${label}<span aria-hidden="true">↗</span></a>`).join('')}</nav></header>`;
 const footer = `<footer class="site-footer"><div class="footer-top"><a class="brand" href="index.html" aria-label="Argys home"><img src="assets/argys-logo.png" width="756" height="416" loading="lazy" alt="Argys — Vision in Surgery"></a><p>Intelligence in the instrument.<br>Possibility in the surgeon’s hands.</p><a class="text-link" href="${escape(config.linkedin)}" target="_blank" rel="noopener noreferrer">LinkedIn <span aria-hidden="true">↗</span><span class="sr-only"> (opens in a new tab)</span></a></div><div class="footer-bottom"><span>© ${config.year} ${escape(config.legalName)}</span><div><a href="sources.html">Sources &amp; credits</a><a href="privacy.html">Privacy</a></div><span>Technology in development</span></div></footer>`;
 const contact = config.contactEmail ? `<a class="button button-dark" href="mailto:${escape(config.contactEmail)}">Email Argys <span aria-hidden="true">↗</span></a><a class="contact-email" href="mailto:${escape(config.contactEmail)}">${escape(config.contactEmail)}</a>` : `<a class="button button-dark" href="${escape(config.linkedin)}" target="_blank" rel="noopener noreferrer">Connect on LinkedIn <span aria-hidden="true">↗</span><span class="sr-only"> (opens in a new tab)</span></a>`;
